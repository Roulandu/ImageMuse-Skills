...
默认主体为“原创成年东方女性，20岁及以上”；当用户明确指定其他安全主体时，技能会尊重用户设定，不强行覆盖。

当用户没有主动指定人物姿态或镜头朝向时，技能默认让人物正面面向镜头，或轻微三分之四侧身但脸部与身体仍自然朝向镜头；镜头默认使用正视或轻微俯视的 editorial 角度。

4 个技能会按意图在 `最终中文提示词` 中加入固定中文风格锚点。基础锚点默认包含成人年龄、成熟丰腴且比例协调的整体体态、自然端正姿态、默认正面或轻微侧身面向镜头、正视或轻微俯视镜头、明亮室内或高级家居自然光场景、优雅合身服装；泳装 Lookbook、女仆/甜美幻想风、回眸姿势和 S 型身材只在用户输入或修复目标匹配时追加。这些锚点用于整体人像、服装版型和高级时尚构图，不用于身体局部特写或低俗凝视。
...
- 把身体局部强调改成整体体态、服装剪裁、默认面向镜头的姿态和正视/轻微俯视镜头语言。
...
其中 `最终中文提示词` 是可以直接复制给 Image2 / GPT Image 2 使用的主体提示词。`负面限制词` 用于约束不希望出现的内容，例如未成年感、幼态、学生元素、真实人物相似、低俗姿势、露骨裸露、身体局部特写、低俗凝视、软色情姿势、手指畸形等。

`最终中文提示词` 也是唯一需要完整承载已触发固定风格锚点的字段；其他字段可以摘要说明，不必重复整段长句。

如果用户没有指定安全姿态或镜头，`最终中文提示词` 应默认包含“人物正面面向镜头，或轻微三分之四侧身但脸部与身体仍自然朝向镜头，镜头为正视或轻微俯视 editorial 角度”一类方向。回眸、背身、明显侧身、仰拍等只在用户明确要求且安全时加入；低机位身体凝视、偷拍、窥视感或身体局部主导构图仍会被改写或移除。
